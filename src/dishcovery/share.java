...
public class share extends javax.swing.JFrame {
    ...
    private void performSearch() {
        ...
        String searchTxt = Search1.getText().trim();
        if (!searchTxt.isEmpty() && !"Search recipes by name or ID".equals(searchTxt)) {
        ...
        String categoryFilter = String.valueOf(category.getSelectedItem());
        if (categoryFilter != null && !categoryFilter.isEmpty() && !"All Category".equalsIgnoreCase(categoryFilter)) {
    ...
    private void viewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_viewActionPerformed
        int row = Sharedtable.getSelectedRow();
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Please select a recipe to share.");
            return;
        }
        String idStr = String.valueOf(Sharedtable.getValueAt(row, 0));
        try {
            int id = Integer.parseInt(idStr);
            con.updateRecord("UPDATE Recipes SET r_status = 'Processing...', r_shared = 1 WHERE r_id = ?", id);
            JOptionPane.showMessageDialog(this, "Recipe shared! Status is now 'Processing...'.");
            performSearch();
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Failed to share recipe: " + ex.getMessage());
        }
    ...
    private void view1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_view1ActionPerformed
         int row = Sharedtable.getSelectedRow();
        if (row < 0) {
            JOptionPane.showMessageDialog(this, "Please select a recipe from the table to view.");
            return;
        }
        Object idObj = Sharedtable.getValueAt(row, 0);
        if (idObj == null) {
            JOptionPane.showMessageDialog(this, "Please select a valid recipe.");
            return;
        }
    ...
    private void SearchActionPerformed(java.awt.event.ActionEvent evt) {
        if (Search != null && Search1 != null) {
            Search1.setText(Search.getText());
        }
        performSearch();
    }
    ...
    private void ADD13ActionPerformed(java.awt.event.ActionEvent evt) {
        if (Search != null) Search.setText("Search recipes by name or ID");
        if (Search1 != null) Search1.setText("");
        if (category != null && category.getItemCount() > 0) category.setSelectedIndex(0);
        if (category1 != null && category1.getItemCount() > 0) category1.setSelectedIndex(0);
        performSearch();
    }
    ...
    private void category1ActionPerformed(java.awt.event.ActionEvent evt) {
        if (category1 != null && category != null) {
            Object sel = category1.getSelectedItem();
            if (sel != null) {
                category.setSelectedItem(sel);
            }
        }
        performSearch();
    }
    ...
    private void SharedtableMouseClicked(java.awt.event.MouseEvent evt) {
